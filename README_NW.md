...
The RAG pipeline was evaluated using [RAGAS](https://docs.ragas.io/) on a set of 6 biology questions with ground-truth answers drawn from NCERT content (implemented in `evaluation.py` via the `evaluate_results` function — backend only, not surfaced in the UI):
